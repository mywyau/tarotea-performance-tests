...
const startDuration = new Trend('dojo_topic_jyutping_start_duration');
const finalizeDuration = new Trend('dojo_topic_jyutping_finalize_duration');
const audioDuration = new Trend('dojo_topic_jyutping_audio_duration');

const sessionsStarted = new Counter('dojo_topic_jyutping_sessions_started');
const sessionsFinalized = new Counter('dojo_topic_jyutping_sessions_finalized');
const audioFilesFetched = new Counter('dojo_topic_jyutping_audio_files_fetched');
...
export const options = {
  ...
  thresholds: {
    ...
    dojo_topic_jyutping_start_duration: ['p(95)<1500', 'p(99)<2500'],
    dojo_topic_jyutping_finalize_duration: ['p(95)<2500', 'p(99)<5000'],
    http_req_duration: ['p(95)<2000'],

    // only meaningful if FETCH_AUDIO=true
    dojo_topic_jyutping_audio_duration: ['p(95)<1500'],
  },
...
export default function () {
  ...
  group('start topic jyutping dojo session', function () {
    const startRes = http.get(
      `${BASE_URL}/api/typing/topic/v2/start?scope=topic&slug=${encodeURIComponent(TOPIC_SLUG)}&variant=jyutping`,
      {
        headers,
        tags: { name: 'GET /api/typing/topic/v2/start?variant=jyutping' },
      }
    );
    ...
    group('simulate topic jyutping typing session', function () {
    ...
    group('finalize topic jyutping dojo session', function () {
