...
const startDuration = new Trend('dojo_sentence_chinese_start_duration');
const finalizeDuration = new Trend('dojo_sentence_chinese_finalize_duration');

const sessionsStarted = new Counter('dojo_sentence_chinese_sessions_started');
const sessionsFinalized = new Counter('dojo_sentence_chinese_sessions_finalized');
...
export const options = {
  ...
  thresholds: {
    ...
    dojo_sentence_chinese_start_duration: ['p(95)<1500', 'p(99)<2500'],
    dojo_sentence_chinese_finalize_duration: ['p(95)<2500', 'p(99)<5000'],
...
export default function () {
  ...
  group('start sentence chinese dojo session', function () {
    const startRes = http.get(
      `${BASE_URL}/api/typing/sentences/v2/start?scope=level&slug=${encodeURIComponent(LEVEL_SLUG)}&variant=chinese`,
      {
        headers,
        tags: { name: 'GET /api/typing/sentences/v2/start?variant=chinese' },
      }
    );
    ...
    group('simulate sentence chinese typing session', function () {
    ...
    group('finalize sentence chinese dojo session', function () {
