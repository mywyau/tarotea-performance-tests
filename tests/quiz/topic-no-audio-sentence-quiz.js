...
const startDuration = new Trend('topic_sentence_quiz_start_duration');
const finalizeDuration = new Trend('topic_sentence_quiz_finalize_duration');

const quizzesStarted = new Counter('topic_sentence_quizzes_started');
const quizzesFinalized = new Counter('topic_sentence_quizzes_finalized');
...
export default function () {
  ...
  group('start topic sentence quiz', () => {
    ...
    const ok = check(res, {
      'topic sentence start 200': (r) => r.status === 200,
    });
  ...
  group('finalize topic sentence quiz', () => {
    ...
    const ok = check(res, {
      'topic sentence finalize 200': (r) => r.status === 200,
    });
